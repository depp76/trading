...
> 저장소(src) 위에서 **CLI(Claude Code) 기반**으로 진행한다 — `trading.md`가 리밸런싱
...
>
> 관련 문서: `trading.md`(주간 포트폴리오 리밸런싱 알고리즘 — 별개 트랙, 서로 코드/문서를
...
- 대상 시장/자산군: 주식·외환·선물/옵션이 최종 범위이나, v1은 주식 단일 종목 백테스트부터
  시작한다.
- `strategy/rebalance/`(`trading.md`)와는 별개 트랙이다 — 리밸런싱은 "여러 종목을 매주
...
| `run_backtest_strategy()` | `strategy/ma_cross.py` (구 `data/backtest.py`) | 개별 종목 단위, 벡터화(numpy), no-lookahead 원칙이 이미 적용된 기존 백테스트 로직 — 이 알고리즘과 "종목 단위 백테스트"라는 성격이 가장 가깝다. 다만 전략 로직(MA20/60 골든크로스)은 다르므로 함수를 공유하지는 않고, 설계 패턴(벡터화 방식, 지표 재사용 방식)만 참고한다. |
...
- `src/data/trend_following/`에 남아있는 문서(`README.md`, `trend_following.md`)는 이번 결정
  이전에 만든 것으로, 이 경로 변경 이후로는 쓰지 않는다 — 폴더째 정리 대상(수동 삭제 필요).
...
- [x] 소스 위치를 `data/trend_following/`이 아닌 `src/strategy/trend_following/`로 확정
      (2026-09-17, `trading.md` 11-5) — `data/rebalance/`·`data/backtest.py` 등 기존 전략 코드도
...
- 2026-09-17: 소스 위치를 `data/trend_following/` → `src/strategy/trend_following/`로 변경
  (4장, 6장) — `trading.md` 11-5에 전체 마이그레이션 계획 수립.
- 2026-09-17: `trading.md` 11-5 실행 완료에 맞춰 경로 표기 갱신 — `strategy/rebalance/`,
