...
**v3 다종목 분산 포트폴리오 (2026-09-17 구현, `portfolio.py`)**
- 종목마다 위 v1/v2 규칙을 독립적으로 적용해 일간 전략수익률(비중·비용 반영)을 얻고, 날짜 합집합으로
  정렬한 뒤 **N개 슬리브 균등 배분(constant mix)**으로 합성한다 — 매일 자본을 1/N씩 나누므로
  포트폴리오 일간수익률은 슬리브 수익률의 단순 평균이다. 어떤 종목이 그날 거래가 없으면(휴장·거래소
  차이) 그 슬리브는 0으로 들어간다.
- 총노출(`gross_exposure`)은 슬리브 비중의 평균이라 `max_weight`를 넘지 않는다(레버리지 없음).
- 산출: 포트폴리오 수익 지표(`return_metrics`)와 `n_instruments`, `avg_gross_exposure_pct`,
  `avg_n_positions`, 종목별 한 줄 요약, 일별 프레임(`daily`), 슬리브별 수익률 프레임.
- 슬리브 간 리밸런싱 비용은 계산하지 않는다(균등 배분 유지에 드는 거래는 무시). 개별 슬리브의
  진입/청산 비용만 반영된다.

**IS/OOS 검증 (`validation.py`)**
- 신호는 항상 전체 이력으로 계산하고(과거만 참조하므로 실거래와 동일) **평가 구간만 날짜로 자른다**.
  구간 시작 전에 열린 포지션은 그대로 이어진다.
- `holdout_validation(histories, split_date, grid)`: 분할일 이전(IS)에서 그리드 최적(기본 목적함수
  Sharpe, 선택적으로 MDD 상한)을 고른 뒤 이후(OOS) 성과를 보고한다. 그리드 전체의 IS/OOS 지표와
  IS→OOS 순위상관도 함께 낸다.
- `walk_forward_validation(histories, folds, grid)`: 앵커드(확장 창) 워크포워드. 각 OOS 폴드(예:
  연 단위)마다 그 이전 전체 데이터에서 최적을 고르고 폴드를 그 설정으로 거래한 뒤, OOS 폴드들을 이어
  붙인 수익률로 최종 지표를 낸다 — **이 수치를 대표값으로 인용한다.**
- 기본 그리드 `DEFAULT_GRID`: entry/exit ∈ {20/10, 55/20, 100/50} × 변동성 타깃 ∈ {off, 15%} ×
  손절 ∈ {off, 3×ATR} = 12개.
...
- **구현 완료 (2026-09-17, CLI 세션)** — 위 구성 그대로 `config.py`(`TrendFollowingConfig`),
  ...
  그대로 사용한다.
- **v3 포트폴리오·검증 (2026-09-17)** — `portfolio.py`(`run_portfolio_backtest`,
  `run_portfolio_backtest_for_tickers`), `validation.py`(`holdout_validation`,
  `walk_forward_validation`, `yearly_folds`, `window_metrics`, `DEFAULT_GRID`).
  `backtest.py`의 수익률 지표 계산을 `return_metrics()`로 분리해 세 곳이 공유한다. 테스트
...
- **v3 UI 연결 (2026-09-17)** — Trend Following 탭 네 번째 입력 행: 포트폴리오 티커 목록(쉼표 구분,
  ...
  "Validate (IS/OOS)"(최근 N년을 연 단위 OOS 폴드로 한 워크포워드 + 첫 OOS 연도를 분할일로 한
  홀드아웃, 그리드는 `DEFAULT_GRID` 12개 → `TrendFollowingValidationDialog`: 폴드 표, 이어붙인 OOS
  에쿼티, 홀드아웃 그리드 표). 두 작업 모두 `threads.fetch_threads.TrendFollowingPortfolioThread`에서
  실행되며 진행 상황을 상태 라벨에 표시한다.
...
- **v2 오버레이 예비 결과 (2026-09-17)** — 같은 6종목·2021-01-01~, Donchian 20/10 고정, 무비용.
  ...
  - 따라서 다음 단계는 단일 종목 규칙 추가가 아니라 **여러 종목 분산 포트폴리오**(상관이 낮은
    ...
    맞추기 어렵다는 것이 v1·v2 결과의 공통 결론이다. → 아래 v3 결과.

- **v3 다종목 포트폴리오 결과 (2026-09-17)** — 14종목(KR 10: 005930, 000660, 035420, 005380,
  051910, 068270, 105560, 000270, 012330, 055550 / US 4: AAPL, MSFT, NVDA, ^GSPC), 2016-01-04~,
  균등 슬리브, 무비용.

  전 구간(in-sample):

  | 설정 | CAGR % | 변동성 % | Sharpe | MDD % | 총노출 % | 게이트 |
  |------|--------|---------|--------|-------|---------|--------|
  | v1 20/10 | +10.7 | 9.5 | 1.09 | 13.6 | 41 | MDD만 통과 |
  | v1 55/20 | +10.0 | 9.9 | 0.98 | 14.7 | 39 | MDD만 통과 |
  | v2 20/10 + 타깃15% | +4.6 | 4.1 | 1.08 | 6.4 | 22 | MDD만 통과 |
  | v2 20/10 + 타깃15% + 손절3 | +4.6 | 3.9 | 1.15 | 6.3 | 21 | MDD만 통과 |
  | v2 20/10 + 레짐200 + 타깃15% + 손절3 | +4.0 | 3.5 | 1.12 | 4.6 | 17 | MDD만 통과 |
  | v2 20/10 + 타깃25% + 손절3 | +7.2 | 6.0 | 1.14 | 10.0 | 31 | MDD만 통과 |

  분산 효과가 뚜렷하다: 같은 v1 20/10 규칙의 종목별 Sharpe는 −0.04~1.07(평균 0.5)인데 포트폴리오
  Sharpe는 1.09, MDD는 13.6%로 단일 종목(27%대)의 절반이다.

  홀드아웃(IS 2016~2022, OOS 2023~2026-09, 기본 그리드 12개, 목적함수 Sharpe):

  | | CAGR % | Sharpe | MDD % |
  |---|--------|--------|-------|
  | IS 최적 = 100/50, 오버레이 없음 — IS | +12.0 | 1.33 | 12.2 |
  | 같은 설정 — OOS | +22.7 | 1.42 | 14.0 |
  | (참고) 100/50 + 타깃15% — OOS | +11.4 | 1.46 | 6.7 |

  IS→OOS Sharpe 순위상관 0.75 — 그리드 12개의 IS 순위가 OOS에서도 대체로 유지되어 과최적화 징후는
  약하다. 채널이 긴 100/50이 IS·OOS 모두 20/10·55/20보다 낫다(단일 종목 v1 결과와 반대 — 종목
  수가 많으면 잦은 신호보다 큰 추세를 오래 타는 쪽이 유리).

  앵커드 워크포워드(OOS 연 단위 2019~2026, 각 해의 설정은 그 이전 전체 데이터에서 선택):

  | OOS 연도 | IS에서 선택 | OOS Sharpe | OOS CAGR % | OOS MDD % |
  |---------|-----------|-----------|-----------|----------|
  | 2019 | 100/50 | 1.55 | +9.3 | 5.0 |
  | 2020 | 100/50 | 1.88 | +26.8 | 12.2 |
  | 2021 | 100/50 | 1.17 | +15.4 | 7.5 |
  | 2022 | 100/50 + 타깃15% | −2.72 | −6.7 | 6.8 |
  | 2023 | 100/50 | 1.68 | +14.4 | 6.6 |
  | 2024 | 100/50 | 1.37 | +18.1 | 10.1 |
  | 2025 | 100/50 | 1.95 | +23.5 | 7.0 |
  | 2026 (~9월) | 100/50 | 1.44 | +42.2 | 14.0 |
  | **이어붙인 OOS 2019~2026** | | **1.25** | **+16.2** | **14.0** |

  선택 시 MDD 15% 상한을 걸어도 결과는 동일(모든 후보의 IS MDD가 15% 이내).

  관찰:
  - **MDD 조건(≤ 15%)은 OOS에서 충족**(14.0%)했고 Sharpe는 1.25로 게이트(1.5)에 미달 — 8개 폴드 중
    4개(2019, 2020, 2023, 2025)는 1.5를 넘었지만 2022년(약세장, −6.7%)이 전체를 끌어내린다. 매년 같은
    100/50이 선택되는 점은 파라미터가 안정적임을 뜻한다.
  - v2 오버레이는 포트폴리오에서도 Sharpe를 높이지 못한다(변동성만 비례 축소). 분산이 v2보다 훨씬
    효과적인 위험 감소 수단이다.
  - 한계: 무비용, 슬리브 간 리밸런싱 비용 미반영, 14종목은 임의 선택(생존 편향 가능성 — 현재
    대형주 위주), 균등 배분. 롱온리라 2022 같은 해에는 현금 외 방어 수단이 없다.
  - 다음 후보: (1) 종목군을 Trading Universe 전체(KOSPI/KOSDAQ 상위)로 확장해 편향 축소, (2) 슬리브
    간 변동성 역가중 배분, (3) 리밸런싱 비용 반영, (4) 2022형 약세장 대응(시장 레짐을 개별 종목이 아닌
    지수 기준으로 적용).
...
- [ ] `entry_n`/`exit_n` 등 파라미터 초기값 확정 — 단일 종목 예비 결과는 20/10, **포트폴리오
      워크포워드는 매 폴드 100/50을 선택**(5장 v3). 코드 기본값은 아직 20/10; 포트폴리오 운용
      기준으로 100/50 채택 여부 결정 필요.
...
- [x] 여러 종목 분산 포트폴리오 백테스트 — v3 `portfolio.py` (2026-09-17, 3장·5장 참고)
- [x] In-sample / Out-of-sample 분리 검증 방식 결정 — 홀드아웃 + 앵커드 워크포워드(연 단위),
      `validation.py` (2026-09-17). 대표값은 이어붙인 워크포워드 OOS: Sharpe 1.25, MDD 14.0%
- [x] v3 UI 연결 — Trend Following 탭 "Run Portfolio" / "Validate (IS/OOS)" (2026-09-17, 4장 참고)
- [ ] 종목군 확장·변동성 역가중·리밸런싱 비용·지수 레짐 (5장 v3 관찰의 다음 후보)
...
- 2026-09-17: v2 오버레이(레짐 MA 필터, ATR 손절, 변동성 타깃 사이징) 구현 및 UI 입력 행 추가,
  ...
  다음은 다종목 분산.
- 2026-09-17: v3 다종목 포트폴리오(`portfolio.py`)와 IS/OOS 검증(`validation.py`) 구현, 14종목
  실데이터로 전 구간·홀드아웃·워크포워드 결과를 5장에 기록. 워크포워드 OOS Sharpe 1.25 / MDD 14.0%:
  MDD 조건 통과, Sharpe 미달. 6장 체크리스트 갱신.
- 2026-09-17: v3 포트폴리오·IS/OOS 검증을 UI에 연결 — 탭 입력 행, `TrendFollowingPortfolioThread`,
  `ui/dialogs/trend_following_portfolio.py`(포트폴리오·검증 다이얼로그).
