...
> 기능을 **멀티 에이전트로 개발하는 방법론**, (3) 신규 알고리즘의 성과와 비교할 기존 수동
> 매매 베이스라인을 한 문서에서 계속 누적해 나가는 통합 문서. 이전에는 개발 방법론을
> `multi_agent_guide.md`에 별도로 뒀으나 12장으로 흡수됨.
> 위치: 2026-09-17까지는 저장소 루트의 `trading.md`였고, 같은 날 코드와 나란히 두기 위해
> `src/strategy/rebalance/rebalance.md`로 옮긴 뒤 `trading.md`는 삭제했다. 절 번호는 그대로
> 유지되므로 코드 주석·커밋 메시지의 "trading.md N-x" 인용은 이 문서의 같은 절을 뜻한다.
...
- 산출 지표: 총수익률, 벤치마크(KS11) 대비 수익률, CAGR, MDD, Sharpe, 연환산 변동성, 승률,
  총 비용/비용 드래그(%). Sharpe/변동성은 `_sharpe_and_vol()`이 매주 금요일 시점의
...
| G | 백테스트 실행 결과를 7장에 기록 | 7장 | `rebalance.md` (문서만) |
...
완료 기준: <예: pytest 전부 통과, 8장 해당 항목에 근거 남길 것>
...
### 12-7. 공용 파일(rebalance.md, ROADMAP.md, requirements.txt 등) 다루는 법

- 이 문서(rebalance.md, 당시 trading.md)처럼 "현재 상태"를 기록하는 문서는 각 에이전트가 완료한 뒤, 병합이
...
- **코드 변경과 문서 갱신을 같은 커밋 사이클로 묶는다.** 12-11에서 실제로 겪었듯,
  이 문서(당시 `trading.md`)는 git 커밋 단위가 아니라 "파일을 직접 읽고 쓰는" 방식으로 다뤄지면 12-2의
...
완료 후 보고: 커밋 해시, 변경 파일 목록, pytest 결과 — rebalance.md는 직접 갱신하지 말 것
...
| 2026-09-17 | 문서 이동: 저장소 루트 `trading.md`를 통째로 `src/strategy/rebalance/rebalance.md`로 옮기고 `trading.md`는 삭제(사용자 판단: 별도 파일 불필요). 처음엔 1~9장·11장·13장만 분리했다가 남은 10장·12장도 같은 날 이 문서로 합쳐 원문 구성(1~13장, 번호 동일)을 유지. 코드 주석·UI 문자열·다른 문서의 `trading.md N-x` 인용은 `rebalance.md N-x`로 갱신. |
